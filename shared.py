...
# Map individual session sources to cache keys (projects+local-agent → claude)
...
def load_sessions(source: str = "all"):
    """Load and enrich all Claude sessions (disk + memory cached).
    ...
    # Check if we can use parallel parsing for 'all' on cache miss
    if source == "all":
        ...
        if len(missing) >= 2:
            # Multiple sources missing — use parallel parse for speed
            sessions = parse_sessions_parallel("all")
            enrich_sessions(sessions)
            # Split back into per-source caches
...
def filter_by_time(sessions, time_range: str = "All time"):
    ...
    (they may predate the registry or come from sources that don't track time).
    """
...
def _prewarm_caches_async():
    ...
    Skips sources already cached (avoids racing with foreground load_sessions).
    """
    def _prewarm():
        try:
            # Check which sources already cached (foreground may have beat us)
            missing = [src for src in ALL_SOURCES if cache_get(f"sessions_{src}") is None]
            if not missing:
                return  # Already cached by foreground

            # Only parse missing sources (not all 4)
            all_parsed = []
            for src in missing:
                s = parse_sessions(source=src)
                enrich_sessions(s)
                all_parsed.extend(s)
            # Split into per-source caches
            ...
            for src in ALL_SOURCES:
                # Double-check: only cache if still missing
                if cache_get(f"sessions_{src}") is None:
                    cache_set(f"sessions_{src}", by_source[src])
        except Exception:
            pass  # Silently fail — prewarming is optional
...
""",
    ...
    with st.sidebar:
        ...
        if st.button("🔄 Refresh data", use_container_width=True):
            st.cache_data.clear()
            cache_clear()
            # Kick off background pre-warming so next page load is instant
